...
function ProductPage() {
  ...
  const [itemAlreadyInCart, setItemAlreadyInCart] = React.useState(false);
  ...
  React.useEffect(() => {
    ...
    if (itemsInCart) {
      itemAlreadyInCartToSet =
        itemsInCart.findIndex((item) => item.id === selectedItem.id) >= 0;
    }
  ...
  const addItemToCart = () => {
    const itemsInCart = JSON.parse(localStorage.getItem("cart"));
    if (itemsInCart) {
      const newItemsInCart = [...itemsInCart, { ...selectedItem }];
      localStorage.setItem("cart", JSON.stringify(newItemsInCart));
    } else {
      const newItemsInCart = [{ ...selectedItem }];
      localStorage.setItem("cart", JSON.stringify(newItemsInCart));
    }
          ...
          <Container>
            <Row>
              ...
              <Col className="ml-auto mr-auto" md="6">
                ...
                <h2 className="main-price">$ {selectedItem.price || 0}</h2>
                <div
